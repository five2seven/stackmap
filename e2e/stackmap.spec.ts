...
test('reviews and explicitly confirms a Portainer import selection', async ({ page }) => {
  ...
  const selectAll = page.getByRole('checkbox', { name: 'Select all services' })
  ...
  await running.getByRole('checkbox', { name: /Running app running/i }).uncheck()
  await selectAll.check()
  await expect(running.getByRole('checkbox', { name: /Running app running/i })).toBeChecked()
  await expect(stopped.getByRole('checkbox', { name: /Stopped app exited/i })).toBeChecked()
  ...
  await secondPage.getByRole('checkbox', { name: 'Select all services' }).check()
  await expect(repeatRunning.getByRole('checkbox', { name: /Running app running/i })).not.toBeChecked()
  await expect(stoppedRetry.getByRole('checkbox', { name: /Stopped app exited/i })).toBeChecked()
  const existingHostId = inventory[0].hostId
  await secondPage.getByLabel('Set host for selected services').selectOption(existingHostId)
  await secondPage.getByRole('button', { name: 'Apply host' }).click()
  await expect(stoppedRetry.getByLabel('Target host')).toHaveValue(existingHostId)
  await secondPage.getByRole('checkbox', { name: /I reviewed this selection/ }).check()
  await secondPage.getByRole('button', { name: 'Import selected' }).click()
